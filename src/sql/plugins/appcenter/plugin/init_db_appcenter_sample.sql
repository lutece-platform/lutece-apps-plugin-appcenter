...
INSERT INTO appcenter_demand_type ( id, id_demand_type, id_workflow, java_class , label, description, question, id_category_demand_type, n_order ) VALUES
    ...
    (2,'sources',100,'fr.paris.lutece.plugins.appcenter.modules.sources.business.SourcesDemand','Gestion des sources','Ce type de demande permet de déclarer les référentiels de site et de composant de votre application. Un référentiel de sources est un espace collaboratif contenant le code source de votre application.Dans cette demande vous pouvez également gérer les accès à ces référentiels.','Avez-vous besoin de créer des réferentiels de sources (SVN, GitHub… ), ou ajouter des droits sur des référentiels existants ?',1,2),
    (3,'jobs',700,'fr.paris.lutece.plugins.appcenter.modules.jobs.business.JobDemand','Intégration continue','Ce type de demande est destiné à la création tâche d''intégration continue. Une tâche d''intégration continue permet d''automatiser la construction de votre application. Un demande de ce type se traduit par la création de deux jobs sur Jenkins - l''un destiné à la documentation - l''autre à la génération des sources.','Souhaitez-vous que les composants techniques de votre application soient ajoutés à l''outil d''intégration continue ? (Jenkins)',1,3),
    (4,'fastdeployapplication',600,'fr.paris.lutece.plugins.appcenter.modules.fastdeployapplication.business.FastDeployApplicationDemand','FastDeploy','Fast Deploy est un outil de DevOps permettant déployer votre application en un clic. FastDeploy pilote les déploiements en s''appuyant sur les APIs fournies par le BIAD (cloudmgr). Il récupére automatiquement les sources de votre application sur le référentil de site de votre application.','Souhaitez-vous que votre application utilise l''outil de déploiement simplifié Fast Deploy ?',1,4),
    (5,'environment',-1,'','Demande d''environnement physique','Ce type de demande est destiné à la création d''environnement physique (ajout de VM)','Votre application a t-elle besoin d''un environnement applicatif ?',2,5),
    (6,'openam',200,'fr.paris.lutece.plugins.appcenter.modules.openam.business.OpenamDemand','Authentification Compte Parisien','L''authentification du Compte Parisien (MonCompte) se base sur un serveur SSO utilisant la technologie OpenAM.','Votre application doit-elle proposer un mode d''authentification "MonCompte" pour l''usager (OpenAM) ?',3,6),
    ...
    (8,'notificationgru',400,'fr.paris.lutece.plugins.appcenter.modules.notificationgru.business.NotificationGruDemand','Notifications vers la GRU','Ce type de demande permet d''obtenir la configuration nécessaire afin d''émettre des notifications (Mails, vue 360, MonCompte). Ces notifications sont visibles par les agents dans le back office de la GRU, et par les usagers dans leur tableau de bord MonCompte.','Votre application doit-elle envoyer des notifications vers la GRU, c''est à dire des notifications vers la vue 360, le compte parisien des usagers, ou envoyer des mails à ces derniers ?',3,8),
    (9,'identitystore',500,'fr.paris.lutece.plugins.appcenter.modules.identitystore.business.IdentitystoreDemand','Référentiel d''identité','Ce type de demande permet d''établir un contrat entre votre application et le référentiel d''identité de la GRU. Il s''agit ici de déterminer quels sont les droits de votre application sur les attributs d''identité des usagers. ( lecture, écriture, certification ). Les trois méthodes pour récupérer des attributs d''identité ci-dessous sont classées par simplicité d''utilisation','Votre application doit-elle récupérer des attributs de l''usager qui se connecte ?',3,9),
...
INSERT INTO appcenter_documentation( id_documentation, id_demand_type, label, url, category ) VALUES
    ...
    (3,6,'Présentation de l''architecture de l''autentification','http://fr.lutece.paris.fr/fr/wiki/gru-authentication.html','project_manager'),
